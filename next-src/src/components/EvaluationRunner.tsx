    ...
    {tab === 'new' && access==='approved' && <form className="evaluation-form eval-workspace eval-deploy" noValidate onSubmit={submit}>
      <div className="eval-main">
        ...
        <section className="eval-section"><header><span>04</span><h2>Compute &amp; run</h2></header>
          ...
          <label className="eval-token">Hugging Face token (optional)<input type="password" value={hfToken} onChange={e=>setHfToken(e.target.value)} autoComplete="off" placeholder="hf_…"/><small>Used only for this run. Your Hugging Face account must have access to gated models. Stored encrypted and removed after compute cleanup.</small></label>
          <div className="eval-generation">
            <p className="eval-subhead">Generation</p>
            <div className="eval-fields eval-fields-3">{(['response','reflection','direct_rating'] as const).map(phase=><label key={phase}>{phase==='direct_rating'?'Rating':label(phase)} token budget<input type="number" min="1" step="1" disabled={!advancedValid} value={collectionOptions.generation?.[phase]?.max_tokens ?? ''} placeholder={{response:'Default: 4096',reflection:'Default: 2048',direct_rating:'Default: 512'}[phase]} onChange={e=>setBudget(phase,e.target.value)}/></label>)}</div>
            <p className="eval-help">Increase reflection tokens for reasoning models. Per-model overrides are available in Advanced configuration.</p>
            {engine==='native'&&<label className="eval-toggle"><input type="checkbox" checked={collectionOptions.failure_policy!=='strict'} disabled={!advancedValid} onChange={e=>setAdvanced(JSON.stringify({...advancedOptions,collection:{...collectionOptions,failure_policy:e.target.checked?'omit_invalid_judgments':'strict'}},null,2))}/><span><strong>Continue after invalid judgments</strong><small>Omit affected samples after retries instead of stopping the run.</small></span></label>}
          </div>
