...
export function EvaluationRunner() {
  ...
  return <>
    ...
    {tab === 'new' && access==='approved' && <form className="evaluation-form eval-workspace" noValidate onSubmit={submit}>
      <div className="eval-main">
        <section className="eval-section"><header><span>01</span><h2>Model panel</h2></header><p>Each model answers the scenarios and judges the responses. {limits.max_models===null?'Choose at least two models.':`Choose 2–${limits.max_models} models.`}</p>
          <div className="eval-model-presets">{models.map(model => <label className="evaluation-model" key={model.id}><input type="checkbox" checked={selected.includes(model.id)} disabled={!selected.includes(model.id) && (limits.max_models !== null && selected.length >= limits.max_models)} onChange={e => setSelected(ids => e.target.checked ? [...ids, model.id] : ids.filter(id => id !== model.id))} />{model.label}</label>)}</div>
